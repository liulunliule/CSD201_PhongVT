...
public class BSTree {
    ...
    public Node search(Node p, String key) {
        ...
        if (p.info.charAt(0) == key.charAt(0)) {
            return p;
        } else if (p.info.charAt(0) > key.charAt(0)) {
    ...
    public void insert(String x) {
        ...
        if (f == null) {
            root = p;
        } else if (p.info.charAt(0) > f.info.charAt(0)) {
            f.right = p;
        } else {
            f.left = p;
        }
...
//    }
    ...
    public void deleteByCopy(String x) {
        ...
        while (q != p) {
            f = q;
            if (q.info.charAt(0) > p.info.charAt(0)) {
                q = q.left;
            } else {
                q = q.right;
            }
...
//            visit(q);
    ...
    static void sumOfLongRootToLeafPath(Node root, int sum,
        ...
        sumOfLongRootToLeafPath(root.left, sum + root.info.length(),
                len + 1);

        sumOfLongRootToLeafPath(root.right, sum + root.info.length(),
                len + 1);
...
//    }
    ...
    boolean isHeap(Node tree) {
        ...
        if (tree != null
                && (tree.left != null && tree.left.info.charAt(0) > tree.info.charAt(0))
                || (tree.right != null && tree.right.info.charAt(0) > tree.info.charAt(0))) {
            return false;
        }
